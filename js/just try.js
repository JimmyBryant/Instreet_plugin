...
(function(window,undefined){
		...
		*******************************/
		...
		var imgReady = (function () {
		    ...
		    return function (image, ready, load, error) {
        ...
        **********************************/
		var cache={
	        ...
	        initData   :function(){
			   ...
			   for(var i=0,len=imgs.length;i<len;i++){
			   	  ...
			   	  imgReady(img,function(){
			   	  	cache.readyHandler(this);
			   	  },function(){
			   	  	var img=imgs[this.insId];				
			   	  	cache.loadHandler(img);
			   	  });
			   }

		    },
		    readyHandler:function(img){              //获取到图片尺寸后callback
		    	var clientImg=imgs[img.insId];
		    ...
		    loadHandler :function(img){			//图片加载完成后callback

				if(img.dataReady!=true){
					setTimeout(function(){cache.loadHandler(img)},40);
				}else{
					var index=img.insId,data=cache.dataArray[index];
